...
def test_owner_creation():
    ...
    assert Owner.objects.filter(name="Loft", country="France").exists() == True
...
def test_payload_creation():
    ...
    assert Payload.objects.filter(satellite__id=sat.id, provider="ESA").exists() == True
...
def test_launcher_creation():
    ...
    assert Launcher.objects.filter(launcher_type="SpaceX", 
                                   launch_date=launch_date).exists() == True
...
def test_user_creation():
    ...
    assert User.objects.filter(username="testuser").exists() == True
